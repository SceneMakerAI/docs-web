...
1. GitHub Actions `workflow_dispatch` (수동 실행, DAILY/ALL 선택) → 8개 DB 병렬 동기화
2. `notion_to_docs_generic.py` → Notion "하위 항목" relation 파악 후:
...
3. `translate_to_en.py` → 변경된 KR `.md` DeepL 번역 → `docs_en/` 저장, 서브디렉토리 `_category_.json` EN 미러링
4. 변경사항 있으면 `chore: Notion 동기화 YYYY-MM-DD` 커밋 → main push
5. main push → `deploy.yml` 트리거 → `npm run build` → GH Pages 배포

### 계층 구조 관련 환경변수

| 변수 | 기본값 | 설명 |
|------|--------|------|
| `NOTION_PROPERTY_SUBITEM` | `하위 항목` | Notion DB의 Sub-items relation 속성명 |
| `NOTION_PROPERTY_PARENT` | `상위 항목` | Notion DB의 Parent item relation 속성명 |
