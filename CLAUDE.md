...
│   ├── sync.sh                    # Notion → blog/ · contribute/ 동기화 후 즉시 push
...
**자동화 워크플로우 (수동 개입 불필요):**
- `sync-develop.yml` — 매일 KST 03:00에 main 콘텐츠를 develop으로 자동 흡수
- `merge-develop.yml` — 매일 KST 11:00에 develop의 코드 변경(docs·docs_en·static 제외)을 main으로 자동 머지 후 배포
