...
Regions Offering G7e** (Measured on 2026-05-19)
...
**Separation of Use Cases Recommended**
...
At runtime, /stg/models/Qwen3.5-122B-A10B-GPTQ-Int ⇒ /mnt/nvme/models/Move the model to Qwen3.5-122B-A10B-GPTQ-Int4 and load the model from the NVMe drive.
...
- `soundfile` (libsndfile binding) · `librosa` (resampling) · `av` (PyAV, container demux) — All three are required
- **You must restart the service** after installation for the changes to take effect (`sudo systemctl restart vllm_omni_i`)
- You must include `mm_processor_kwargs: {"use_audio_in_video": true}` in the client request body for the audio in the MP4 to be

<br />

processed as well### Service Registration
