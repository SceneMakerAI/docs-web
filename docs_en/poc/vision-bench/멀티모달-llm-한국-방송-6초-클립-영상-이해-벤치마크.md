---
title: "Benchmark for Multimodal LLM Understanding of 6-Second Clips from Korean Broadcasts"
sidebar_position: 1
slug: "1"
---
...
PoC for benchmarking the video understanding quality of multimodal LLMs across 7 Korean TV genres × 100 clips × 6-second clips (700 clips total)

**Test Environment:**

vLLM serving on a single cloud GPU instance / Qwen3-Omni-30B-A3B-Instruct (OpenAI-compatible endpoint)
...
(A) Video Segmentation  — In-house FFmpeg script (separate)
  Splits the 00:10:00 ~ 00:20:00 segment of the original into 100 clips, each 6 seconds long.
  File names are encoded with the original absolute timestamp (e.g., 0001_0600-0606.mp4) to prevent conflicts even if the time window changes.
...
(B) Dialogue Script  — scripts.json, manually created
  ...
  ※ Automatic retrieval via integration with external subtitle and STT systems is planned for the future.
...
Send the MP4 base64 data URI (video_url) + text prompt as an OpenAI-compatible `chat.completions`
in a single request. Audio is embedded within the MP4 and decoded simultaneously via the vLLM video pipeline.
⬇️
Step 2. JSON Schema-Enforced Response (Guided Decoding)
Force the vLLM to return a response in the format `response_format=json_schema(strict)` with the three fields: {summary, objects, actions}.
...
- **Reason:** Since the vLLM video pipeline decodes video and audio simultaneously, there is zero additional cost for separation. The separate input method cannot currently be run because the `vllm[audio]` decoders (`av` / `soundfile` / `librosa`) are not installed in the server vLLM venv. — The output (frames + audio.wav in `data/derived/`) is preserved, allowing immediate resumption once server dependencies are strengthened.
...
- **Reason:** Can be stored and consumed as-is without post-processing (parsing, cleaning, adding/removing fields) code. Analysis is limited to a 6-second 'current' clip, with preceding, current, and following dialogue attached; however, the prompt explicitly states, "The preceding and following dialogue are for context only; do not incorporate them into the description." The two issues discovered during the `from_video` validation phase (summary copying vision text verbatim / prompt rule text mixed into the audio field) have been addressed in the field-specific guidelines.
...
| `objects` (array of string) | Noun keywords such as objects, people, subtitles, and logos appearing in the video (no duplicates; each entry within 3 words). |
...
- **Analysis API:** Internal FastAPI server `POST /analyze/by-clip-path` — Send `clip_path` + `script_prev/curr/next` via form → Response envelope `{result: {summary, objects, actions}, meta: {model, elapsed_ms, usage}}`

- **Concurrency / Backpressure:** `VLLM_CONCURRENCY=4` (`asyncio.Semaphore`) — Excess requests are queued without being rejected

- **Storage:** Results are saved to `predictions/{category}/{source_name}/{clip_id}.json`, followed by qualitative comparison using random samples per category
