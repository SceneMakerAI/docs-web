...
- **Reason:** It is nearly the only open-source option capable of processing all four modalities—Image, Video, Audio, and Text—with a single model and supporting OpenAI-compatible vLLM serving. Its MoE architecture (30B total / 3B active) enables single-GPU inference, and vLLM guided decoding ensures that JSON Schema-compliant responses are received as-is.

<br />

#### **2.2. Input Method:** `from_video` **(single MP4 input) vs** `from_frames_audio` **(separate inputs)**

- **Conclusion:** Adopted the `from_video` method, which passes a single 6-second MP4 clip as a single `video_url` (base64 data URI) component. Separate input (`from_frames_audio`) is **on hold**.
...
- **Reason:** Can be stored and consumed as-is without post-processing (parsing, cleaning, adding/removing fields) code. Analysis is limited to a 6-second 'current' clip, with preceding, current, and following dialogue attached; however, the prompt explicitly states, "Do not incorporate preceding or following dialogue into the description—use them only for context." The two issues discovered during the `from_video` validation phase (summary copying vision text verbatim / prompt rule text mixed into the audio field) have been addressed in the field-specific guidelines.
...
| **Total** | — | **700** | 7 original videos (1 per genre, 10-minute window divided into 100 equal parts) |
