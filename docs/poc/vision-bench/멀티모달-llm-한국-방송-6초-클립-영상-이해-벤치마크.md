...
```bash
uvx yt-dlp -f "bv*[ext=mp4]+ba[ext=m4a]/b[ext=mp4]/b" --merge-output-format mp4 -o "data/raw/news/%(title)s.%(ext)s" "&lt;테스트 대상 URL&gt;"
```

- `-f "bv*[ext=mp4]+ba[ext=m4a]/…"` : **H.264+AAC mp4 우선** (vLLM/ffmpeg 디코딩 호환). 해당 포맷 없으면 최고품질로 폴백(`/b` )

- `--merge-output-format mp4` : mp4 컨테이너로 병합 (**재인코딩 아님** → 원본 품질 유지)

- **② 6초 클립 분할** (사내 ffmpeg 스크립트, 본 PoC 범위 외) — `00:10:00~00:20:00` 구간을 6초 100클립으로 → `data/clips/{category}/{원본명}/{seq}_{start}-{end}.mp4`
