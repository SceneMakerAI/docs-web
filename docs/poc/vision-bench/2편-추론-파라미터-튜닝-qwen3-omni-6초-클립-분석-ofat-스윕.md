...
SceneMaker의 클립 영상 분석은 영상에 포함된 **시각·청각 정보를 안정적으로 생성** 해내는 것을 목표로 한다. 6초 클립 하나를 입력받아 아래 세 갈래의 정보를 `{summary, objects, actions, audio}` 4필드 JSON으로 구조화한다.

- **종합 분석** — `summary` : 시각·청각을 종합한 한 문장 요약
- **시각 분석** — `objects` (객체·인물·자막·로고)와 `actions` (행동·움직임·장면 전환)
- **청각 분석** — `audio` : 시각과 별개로 명확히 들리는 대사·효과음·배경음

여기서 관건은 **안정성** 이다. 700개 클립을 일괄 처리하는 벤치마크에서 같은 클립·같은 프롬프트인데도 실행마다 출력이 흔들리거나 무너지면 품질 점수 자체를 신뢰할 수 없다. 1편(파이프라인 구축) 검증 과정에서 세 가지 품질저하 패턴이 간헐적으로 식별됐다.

1. **빈 응답** — 응답 content가 빈 문자열로 끝나 분석이 통째로 비어 버린다.
2. **degeneration(붕괴)** — `시гля … Arial TTF` 처럼 이종문자가 섞이고 JSON이 미완으로 끝난다.
3. **추론 시간 편차** — 같은 클립인데도 어떤 호출은 즉시 끝나고 어떤 호출은 토큰 상한까지 늘어져, 클립당 추론 시간의 최소\~최대 격차가 컸다.

**빈 응답은 1편에서 strict JSON Schema 강제로 해소** 됐고(첫 토큰을 `{` 로 고정해 조기 종료를 막음), 그에 따라 폭주성 출력이 잡히며 **추론 시간 편차도 크게 줄었다** . 다만 degeneration은 고온 등 특정 설정에서 잔존하고, schema가 출력을 배열 구조로 강제하면서 이번엔 그 배열 안에서 **반복** (같은 항목 도배)과 **파편화** (한 나레이션이 여러 조각으로 분산)가 새로 드러났다. 따라서 본 편은 schema를 켠 상태를 전제로, **반복·파편화·degeneration** 세 양상이 추론 파라미터로 통제되는지를 한 번에 하나씩(OFAT, one-factor-at-a-time) 크게 흔들어 규명한 **1단계 스크리닝** 의 기록이다.
