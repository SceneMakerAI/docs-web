...
| **파라미터** | **특성** | **구간별 특성** | **결론** |
| --- | --- | --- | --- |
| **temperature** | 중기 최적형 | 0.0\~0.3 정체(ok \~83%·foreign 0) **0.3\~0.7 개선** (penalty 7.5% 최저) 0.7\~1.0 완주↑·foreign 35% 폭발·penalty 2배 | **0.7 확정** · 0.3 정밀도 비교는 안전장치 |
| **top_k** | 중기 최적형 | k=1 과조임(ok 82%·finish 18%) **k≈10 정점** (ok 92.5%·penalty 6.7%) k=50\~무제한 완만 후퇴(ok 88\~91%) | 기본값 -1 · **k=10 후보** |
| **top_p** | 후기 최적형 | 0.5 최악(ok 79%·finish 21%) 0.8\~0.95 점진 개선(ok 85→87.5%) **1.0 최선** (ok 90%·finish 10%), 내부 정점 없음 | **1.0 (off)** |
| **frequency_penalty** | 초기 최적·후기 붕괴형 | **0\~0.5 급개선** (ok 90→99.6%·penalty 7→2%) 0.5\~1.0 순응 포화(이득 노이즈)·sounds 79→65% 1.0\~2.0 붕괴(sounds 43%) | **0\~0.5 후보** · >1.0 손해 |
| **repetition_penalty** | 초기 최적·후기 붕괴형 | 1.0\~1.1 완주 개선(ok 90→99.6%) **1.1\~1.3 foreign 1.8→7.2%·actions 100→78% 붕괴** | **제외** |
...
본 단계는 **출력만으로 판정 가능한 것** 의 경계까지 OFAT 로 좁혔다. 확정한 값과 다음 문서로 넘길 미세조정 구간은 아래와 같다.

| **파라미터** | **결론** | **상태** |
| --- | --- | --- |
| **temperature** | 0.7 — greedy 폭주·1.0 이종문자 동시 회피 (추후 개선 여지) | ✅ 확정 |
| **frequency penalty** | 0\~0.5 — 완주·반복 동시 개선, 단 sounds 감소 | ⚠️ 구간 |
| **top_p / top_k** | 1.0 / -1 유지 (top_k 10 완주 후보) | ✅ 확정 |
| **repetition penalty** | 제외 — 이종문자 유발 | ✅ 탈락 |
| **max_tokens / dedup** | 512 / ON | ✅ 확정 |
| **fps** | 0.5 잠정 — temp 0.7 anchor 재측정 예정 | ⚠️ 잠정 |

**다음 문서로:** 확정값을 출발점으로, 구간(frequency penalty·top_k·temperature)과 OFAT 가 드러낸 수치 특성을 종합해 최종값을 정한다. 완주를 더 얻는 대가로 환각·커버리지가 줄어드는지는 출력만으론 못 가리므로 거기서 판정한다. **측정 가능한 것만 결론짓는다** 는 원칙은 유지.

| **파라미터** | **값** | **근거** | **확정도** |
| --- | --- | --- | --- |
| `temperature` | **0.7** | 완주 88%(≈61/70)·finish_len 12.5%·이종문자 0\~0.35%·penalty 최저 7.5%. greedy 는 결정론도 보장 안 됨 | ✅ 확정 |
| `top_p` / `top_k` | **1.0 / -1** (top_k 10 후보) | 조이면 완주율만 하락 — full sampling 유지. 단 top_k 10 은 완주 일관 우위(92.5%)로 다음 문서 미세조정 구간 후보 | ✅ baseline 확정 (top_k 10 미정) |
| `frequency_penalty` | **0.0** (잠정) | freq 0.5 가 완주 90➡️99.6%·penalty 7➡️2% 개선하나 sounds 커버리지 감소 동반 — 득실은 정답지 필요 | ⚠️ 미정 (freq 0\~0.5, 다음 문서서 확정) |
