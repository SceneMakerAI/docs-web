...
- **OFAT** : 한 파라미터만 크게 바꾸고 나머지는 전부 고정한 채 그 변수의 영향만 관찰한다.
- **샘플링 격리** : 페널티 스윕은 greedy(temperature=0)에서, top_p·top_k 스윕은 temp=0.7 anchor 에서 돌린다 — temp=0 이면 top_p·top_k 가 무력(inert)해 효과를 볼 수 없기 때문이다.
