...
한국 방송 7개 장르 × 100개 × 6초 클립(총 700개)에 대한 멀티모달 LLM 영상 이해 품질 벤치마크 PoC

**검증 환경:**

AWS EC2 GPU 1장 (us-west-2) 기반 vLLM 서빙 / Qwen3-Omni-30B-A3B-Instruct (OpenAI 호환 엔드포인트)
...
- **PoC 검증 범위:** 멀티모달 LLM 호출 · 구조화 응답 강제 · 결과 평가로 한정. **영상 분할(ffmpeg)과 대사 스크립트 수집은 사전 준비 단계로 별도 수행** (분할은 사내 ffmpeg 스크립트로 사전 생성, 스크립트는 수동 작성) — 추후 SceneMaker 본 파이프라인 및 외부 자막 · STT 시스템 연동으로 자동화 예정.

- **핵심 목표:**
  1. JSON Schema 강제 (vLLM guided decoding + pydantic `extra="forbid"` ) 로 후처리·정제 단계 제거

  1. **전·현·후 6초 대사** 컨텍스트 동봉 패턴으로 분석 대상 외 내용을 끌어 쓰는 환각(앞·뒤 장면 묘사 혼입) 억제
