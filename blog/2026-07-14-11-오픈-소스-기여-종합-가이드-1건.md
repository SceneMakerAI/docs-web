...
한 줄로 요약하면 이렇다. **"Chat API에서는 되던 기능이 옛날 completions API에서는 안 되던 문제"** 를 메꾼 작업이다.

 코드로는 딱 두 줄, 테스트까지 합쳐도 39줄짜리 작은 변경이지만, 오픈소스에서 "기능 일관성(feature parity)"을 맞추는 일이 왜 중요한지 잘 보여주는 사례라 기록으로 남긴다.
...
코드 두 줄짜리 변경이지만, 이 작업이 전하는 교훈은 규모보다 크다.
...
- **작은 기여가 진입 장벽이 낮다.** 기존 패턴을 복제하고, 하위 호환을 지키고, 테스트로 증명하면 — merge까지 가는 길이 짧다.

큰 기능만 기여인 게 아니다. 이렇게 문 하나를 마저 열어주는 일도, 그 API를 쓰는 누군가에겐 꼭 필요했던 변경이다.
