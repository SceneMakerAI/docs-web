---
title: "Qwen3-Omni baseline — news (without STT, 30 fps vs. 0.5 fps)"
sidebar_position: 2
slug: "2"
---
...
> Measured using two versions: **30 fps (original) vs. 0.5 fps (low-frame-rate version)** — comparing the impact of video frame rate on analysis performance.
...
- Clip naming: `0001_0600-0606` ~ `0100_1194-1200` (Encoded using absolute seconds from the original video)
...
All failure cases resulted in an HTTP 500 Internal Server Error (temporary error on the vLLM side). Usually resolved upon retry.
...
Most prompt tokens consist of mp4 base64 encoding (video frames + audio). At 0.5fps, the number of video frames is reduced to 1/60, resulting in a **6.89× compression** of prompt tokens.

### 6. Processing Speed vs. Real-time

| variant | Total video | wall-time | vs. real-time |
| --- | --- | --- | --- |
| 30fps | 600 sec | 191 sec | **3.14×** |
| 0.5fps | 600 sec | 78 sec | **7.69×** |

### 7. 30fps vs 0.5fps at a Glance

| Metric | 30fps | 0.5fps | Difference |
| --- | --- | --- | --- |
...
Analysis results for the same clip (0050 — industrial statistics graphics scene) at two different frame rates:

**30fps** :

> This broadcast scene shows graphics displayed against an industrial site background, depicting raw materials such as steel and aluminum, as well as derivative products like automobile and aircraft parts. In the lower right corner, a sign language interpreter is performing sign language.
...
→ The key graphics and subtitle content are accurately depicted in both fps. **The difference lies in the dynamic element in the bottom right corner** — 30fps accurately shows "a sign language interpreter performing sign language," while 0.5fps shows "a man waving his hand," **misinterpreting the meaning of the continuous motion**.

#### 8.1 Observation Summary

| Aspect | 30fps | 0.5fps |
| --- | --- | --- |
| Graphics / Captions / Static Visual Elements | Accurate | Accurate (3 frames are sufficient) |
| Character Attire / Background Details | Accurate | Some details missing |
...
| Conclusion | Content |
| --- | --- |
| **Speed** | 0.5fps is **2.40× faster and reduces prompt tokens by 6.89×** — Clear advantage in terms of cost and throughput |
| **Quality** | Static elements such as subtitles, logos, and character positions are equivalent. However, **scene transition and continuous motion recognition capabilities are poor**, with some hallucinations |
...
- [ ] Measure the remaining 6 categories under the same conditions (especially action-heavy `baseball` and `lol` — the 0.5fps gap is expected to be larger)
