---
title: "09_Database Schema Release"
...
This document describes the data structure used by 

---

SceneMaker to analyze and store broadcast video data. (Note: A link to the GitHub

<!--truncate-->

DB schema is required.

> ) The storage formats include structured and unstructured data. The databases used in this project are listed below. 

|  | MariaDB (Relational) | Milvus (VectorDB) |
| --- | --- | --- |
| Content | Structured data. AI results in a **readable format** (subtitles, summaries, people, emotions) | Unstructured data. text **embedding vectors** + metadata for search |
...
We use MySQL and Milvus as data storage systems, and data is stored according to the flow below.
...
Contains the metadata and final summary for each video.

| Column | Type | Description |
| --- | --- | --- |
| `v_id` | mediumint | **PK**, Video ID |
| `p_v_id` | mediumint | Series Link (e.g., Season 2 → Season 1 reference) |
...
| `reg/upd_datetime` | datetime | Date and Time of Entry/Modification |

 #### t_category — Category (self-referencing hierarchy)

It points to the parent category via `p_cate_id` to create a hierarchy such as “Sports → Baseball.”
...
STT results. These are chronological dialogue entries where `(v_id, idx)` is the primary key.
...
Results of video analysis divided into 6-second segments. This is the smallest unit for searching.
...
| `adv` | tinyint | Ad Presence |
| `meta` | JSON | Category-Specific Structure Attributes (Sports=Score/Inning…) |
...
`level` distinguishes between scenes (2) and acts/acts (1), and `parent_chap_id` links scenes to acts.
...
#### t_scene — Event-based scene

Event-based units by genre (e.g., sports = home run, strikeout, etc.). Contains structural attributes for search filters in JSON format.
...
A vector collection that embeds search targets (subtitles, screens, scenes, subtitles) from > relational data.
Since this is a semantic search, unstructured data was used.
...
`ref_type` indicates which table it is, and `ref_id` points to a part of that table’s primary key (`seg_id` /`chap_id` /`idx`).
