...
SceneMaker is releasing the data structure it uses to analyze and store broadcast video—

> including a link to the GitHub

<!--truncate-->

DB schema. The storage formats include structured and unstructured data. The databases used in this project are listed below. 
...
t_video ─┬─ t_dialogue / t_dialogue_refine   (Dialogue)
         ...
         ├─ t_chapter (Scene/Sub)
         ├─ t_scene (scene by incident)
         ├─ t_category  (Category, Self-Referencing Hierarchy)
         └─ t_code (status code)
```

Let’s examine the structure of each database.
...
| `reg/upd_datetime` | datetime | Input/Modification Time |
...
STT results. Dialogue entries in chronological order, with `(v_id, idx)` as the primary key.
...
#### t_segment — 6-second frame analysis (search unit)

Frame analysis results with video segmented into 6-second intervals. This is the smallest unit for searching.
...
| `adv` | tinyint | Advertisement presence |
...
Each vector in Milvus is linked 1:1 to a row in the relational source via `pk = v_id : ref_type : ref_id`.

```text
Milvus "42:segment:120"  ←→  RDB t_segment (v_id=42, seg_id=120)
```

`ref_type` indicates which table, and `ref_id` points to a part of that table’s primary key (`seg_id` /`chap_id` /`idx`).
...
Once all relational storage is complete, vector indexing is performed last.
