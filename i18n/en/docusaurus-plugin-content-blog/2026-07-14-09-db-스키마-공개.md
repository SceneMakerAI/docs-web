...
This document describes the data structures used by 
...
.) The data is stored in both structured and unstructured formats. The databases used in this project are listed below. 
...
Relational tables are connected via `t_video` to `v_id`. 

```text
t_video ─┬─ t_dialogue / t_dialogue_refine   (Dialogue)
         ├─ t_segment  / t_segment_refine    (6-second video analysis)
         ├─ t_chapter (Scene/Sub)
...
#### t_segment — 6-second frame analysis (search atom)

Frame analysis results with video divided into 6-second segments. This is the smallest unit for search.
...
| `meta` | JSON | Structure attributes by category (sports = score/inning…) |
...
Event units by genre (e.g., sports = home runs, strikeouts, etc.). Contains structural attributes in JSON format for search filters.
...
In addition, there are legacy tables: `t_play` (baseball-specific, replaced by `t_scene`) and `t_segment_2` (for experimental use).
