...
To summarize in one sentence: This fix addresses the issue where **"a feature that worked in the Chat API didn’t work in the old completions API"**.

 Although it’s a small change—just two lines of code, or 39 lines including tests—I’m documenting it here as a good example of why achieving “feature parity” is so important in open source.
...
For example, when an in-house chatbot must not generate certain prohibited words or competitors’ names, rather than relying on prompts to manage this, we set the probability of those tokens appearing to zero right at the sampling stage. This is much more reliable and harder to circumvent.
...
I set the **default value for `default_factory=list` to an empty list**. If no value is sent, it behaves exactly as before—allowing any word to pass through. I made sure this matches the default value of the chat endpoint exactly so that both APIs behave identically.
...
def test_completion_request_bad_words_to_sampling_params():
    """Is `bad_words` passed to `SamplingParams` (same behavior as the chat)?"""
...
a two-line code change, the lesson it teaches goes beyond its scale.

- **Engine functionality and API exposure are separate.** Even if a feature is supported internally, if there’s no way to access it, it’s effectively “non-existent” to the user.
- **Consistency is user experience.** If something works in one endpoint but not in another, that in itself feels like a bug.
- **Small contributions have a low barrier to entry.** If you replicate existing patterns, maintain backward compatibility, and validate with tests—the path to a merge is short.
