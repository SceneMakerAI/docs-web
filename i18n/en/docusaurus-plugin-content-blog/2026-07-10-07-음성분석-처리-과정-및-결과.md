...
run()
 ...
 ├ 2) _classify_languages  Language Identification (LID) by Interval
 ├ 3) _transcribe_batched  ◀── This is the actual speech-to-text (ASR) process
 ├ 4) _assemble_segments   Reassembly of words into sentences + speaker mapping
 └ 5) Save JSON
```
...
- If the audio is long, the spectrogram may grow too large, potentially causing a GPU out-of-memory (OOM) error.
  - Set CNUNK_SEC to 30 (maximum chunk length is 30 seconds)

  - chunk = 30 × 48,000 (30 seconds converted to the number of samples)

  - **Conclusion**
    - Increasing this value boosts throughput per chunk but puts VRAM at risk

    - Decreasing this value reduces throughput per chunk but keeps VRAM safe
...
- Identify the segments where a person is speaking and set the sampling rate to 16 kHz
- Sample Input/Output

```python
Input:  [Silence····Word·······Silence··WordWord····Silence]  ← Sound array # np.ndarray (float32 1D)
Output:  [(2.1, 5.8), (40.3, 47.2), ...] ← Only the timestamps of the spoken intervals # list[tuple[float, float]]
```
...
> def transcribe_batched(audio: np.ndarray, language: str) - list[dict]:
    ...
    Returns: [{"start", "end", "word", "seg_logprob"}, ...]  (word-level, absolute time)
             seg_logprob = avg_logprob of the segment to which that word belongs (for the hallucination filter)
    """
    if _model is None:
        load_model()
...
- Currently, only the operation process has been documented. 
...
*This article presents the results of research conducted with support from the Ministry of Science and ICT and the National IT Industry Promotion Agency’s “2026 Open Source AI and Software Development and Utilization Support Program.”*
