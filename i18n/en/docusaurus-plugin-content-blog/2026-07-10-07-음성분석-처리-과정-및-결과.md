---
title: "07_Speech Analysis Process and Results"
...
This article provides a detailed explanation of the speech-to-text conversion process.

<!--truncate-->

It demonstrates how the output is formatted as JSON.

The speech analysis process can be summarized most simply as follows

1. Noise removal (Denoise)
1. Detection of human speech segments (VAD)
1. Speaker separation (Pyannotate)
1. Language detection (LID)
...
run()
 ...
 ├ 1) vad.detect utterance segment (Silero VAD)
...
> def process(audio_np: np.ndarray, sr: int) - tuple[np.ndarray, int]:
    ...
    # === Set the chunk time to 30 seconds, then proceed ===
    
    if total <= chunk:
        # Function to actually perform noise removal <- Noise removal is performed in this section
...
- Since DF (DeepFilterNet) requires a sampling rate (sr) of 48 k, temporarily convert from 16 k to 48 k. 
  - Downsampling to 16 k will be required for future calls

  - Used only during ASR (transcription). Must not be used during LID or VAD segments. 

- If the audio is long, the spectrogram may become too large, potentially causing the GPU to run out of memory (OOM).
  - Set CNUNK_SEC = 30 (maximum chunk length is 30 seconds)

  - chunk = 30 × 48,000 (30 seconds converted to the number of samples)

  - **Conclusion**
    - Increasing this value boosts throughput per chunk but risks VRAM exhaustion

    - Decreasing this value reduces throughput per chunk but ensures VRAM safety
...
- Extract only the portions where a person is speaking
...
- Detect speech segments and set the sampling rate to 16 kHz
- Sample Input/Output

```python
Input:  [Silence····Word·······Silence··WordWord····Silence]  ← Sound array # np.ndarray (float32 1D)
Output:  [(2.1, 5.8), (40.3, 47.2), ...] ← Only the times of the spoken intervals # list[tuple[float, float]]
```
...
```python
 ...
 def diarize(audio_np: np.ndarray, sr: int = 16000) -> list[tuple[float, float, str]]:
    """Extracting the speaker's turn timeline.
...
- LID process for wav audio files.
...
> def transcribe_batched(audio: np.ndarray, language: str) - list[dict]:
    """Transcribe the audio into the specified language (using an internal VAD to extract only the speech segments, with a 30-second window in parallel).

    "Audio" typically refers to "a full-length stream with sections outside the target language muted" → the internal VAD
    Since silence is skipped, only the spoken words are recognized, and the timestamp remains the same as the original time.

    Returns results at the word level using `word_timestamps` → The caller (`stt_service`) uses speaker turns and punctuation as reference points
    Resegmentation (since segments are grouped into 30-second blocks, this restores granularity and speaker accuracy).

    Returns: [{"start", "end", "word", "seg_logprob"}, ...]  (word-level, absolute timestamps)
...
Analysis of the extracted data revealed issues with the subtitles.
