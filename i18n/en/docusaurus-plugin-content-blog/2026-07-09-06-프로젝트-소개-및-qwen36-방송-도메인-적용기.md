...
At a broadcast station, it takes **a few days**—based on a professional editor’s workload—to create a single “Character Roundup” video. To produce these for all five main characters in a 16-episode drama, it would take **several weeks**.
...
OpenWeight model and adapt it to the broadcasting domain using **RAG and prompt engineering**.
...
| **Remix / Short-Form** | Automatic conversion of trailers, highlight clips, and memorable quote cards for each platform |
...
This seems a bit complicated, so I’ll provide an explanation for each step.
...
**Instead of just feeding videos to the VLM, we decided to select only the most relevant content to present to it.**

Afterward, we decided to optimize performance using the tools listed below.

| Section | Tool | Function |
| --- | --- | --- |
| Video/Audio Separation | **FFmpeg** | Separates frame data from the audio stream |
...
| Image Analysis & Summarization | **Qwen3.6** | Inferred scene meaning by integrating the above results |
...
Shot   Conversation between mother and daughter → Close-up of the daughter → Expression just before she starts crying
...
Since it is a graph-based cyclic structure rather than a simple linear pipeline, **it is said to automatically return to a previous step and retry if the results are unsatisfactory**. 
...
| **Instance Name** | **GPU Type** | **Number of GPUs** | **Total GPU Memory (VRAM)** | **Memory Specifications** |
| --- | --- | --- | --- | --- |
...
The following four agents have been built as of today. 

| Agent | Role | Key Stack |
| --- | --- | --- |
| **agent-stt** | Speech recognition, speech extraction by speaker, subtitle generation | faster-whisper, demucs |
...
In this post, I covered **what we plan to build and how we plan to build it**.
