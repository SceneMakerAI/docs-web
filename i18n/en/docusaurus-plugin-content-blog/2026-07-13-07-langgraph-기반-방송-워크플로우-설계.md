...
This article provides a detailed description of the LangGraph process for finding segments of broadcast clips using natural language queries.
...
1. Determine which episode (scope)
2. Compile a list of what to search for (plan)
3. Retrieve similar segments (retrieve)
4. Examine the surrounding context (expand)
5. Determine the exact cut points (select)
6. Merge and adjust the length (assemble)
...
ainvoke(request)
 ├ Scope episode selection (specify v_ids or voting + LLM)
 ├ Plan Route + Beats (by episode)
 ├ retrieve   multi-query vector search
 ├ expand     Hit ±12 seconds context
 ├ select     start–end confirmed (or one material sample)
...
##### 1. scope — Which episode?

- Since this is a series, we first determine “which episode we’re talking about.”
- If `v_ids` exists, use it as-is. If not, conduct a chapter vector vote → finalize using an LLM.
...
> async def run(state: dict, deps) - dict:
    ...
    if req.get("v_ids"):
        ...
        scope = {"mode": "user", "reason": "v_ids specified in the request", "votes": []}
...
```python
Input:  query="Goal-scoring highlights from Game 7 of the Korean Series", v_ids=None
...
A total of 40 votes are spread across 6 episodes. Since the top choice has only 12 votes, it’s difficult to make a decision based on the votes alone, so the LLM is finalized as a single option.
...
# route:
#   pinpoint   — Highlights Clips, Memorable Lines Cards
...
Output:
{
  ...
  "beats": [
    {
      "title": "A Walk by the Lake and a Tender Glance at the Bus Stop",
      "queries": [
...
- “Cast” uses a soft filter (since there are many empty cast rows, a hard filter would drastically reduce recall).
...
- A 6-second hit isn’t a clip. Attach the segment and dialogue from ±12 seconds (two segment slots) before and after.
- `select` provides the material to snap to the boundaries.
...
[
  ...
  {"ref_type": "segment", "ref_id": 26, "start_sec": 150, "end_sec": 156, "text": "A man and a woman by the lake..."},
...
- If irrelevant, `usable=false` + `retry_query` → After re-searching and re-expanding, re-evaluate **only once** (`REFINE_MAX=1`).
...
Output:
{
  ...
  "caption": "Two people stop during a walk by the lake and exchange awkward yet heart-fluttering glances",
  "score": 0.92,
...
Output:
{
  "usable": false,
  "score": 0.0,
  "reason": "The candidate's timeline contains no actual scoring or batting plays, only commentary",
  "retry_query": "KIA Druwa's hit / KIA Yukemi's hit / ..."
}
```
...
##### 6. assemble — Combining and Length Adjustment

- Collect only usable clips. No LLM used.
- Merge overlapping clips from the same episode → duration budget (score-priority) → sort → final JSON.
...
I’ve summarized the search flow and the process of structuring the `clips[]` array, which 

---

were my goals.

The steps for cutting clips into actual video segments (ffmpeg serving) and integrating with the service UI will be covered next.
