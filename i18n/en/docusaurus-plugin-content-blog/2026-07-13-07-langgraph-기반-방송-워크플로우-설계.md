...
This article details the LangGraph process for

---

finding segments of broadcast clips using natural language queries.
...
. Determine the exact start and end points (select)
6
...
A total of 40 votes are spread across 6 episodes. Since the top choice received only 12 votes, it’s difficult to decide based on the votes alone, so the LLM finalizes it as a single option.
...
```python
BUDGET_TOLERANCE = 1.15   # Multiplier for allowing the target duration (in seconds) to be exceeded
...
We’ve covered the search flow and the

---

process of structuring the `clips[]` array, which were our goals.
