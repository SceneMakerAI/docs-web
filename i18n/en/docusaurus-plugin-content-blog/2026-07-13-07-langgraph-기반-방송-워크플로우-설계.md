...
ainvoke(request)
 ...
 ├ select     start–end confirmed (or one attempt at re-submission)
 └ assemble   Overlay Merge · budget · clips[] JSON
```
...
> async def run(state: dict, deps) - dict:
    ...
    # ② Automatic Selection: Chapter Voting → LLM Finalization
        ...
        # ... LLM determines v_ids (up to SCOPE_MAX_VIDEOS)
...
A total of 40 votes are spread across 6 episodes. Since the top choice received only 12 votes, it’s difficult to decide based on the votes alone, so the LLM finalizes it as a single choice.
  ...
  "beats": [
    {
      ...
      "queries": [
        ...
        "A close-up of an exchange of glances that captures the excitement of the early episodes of *Winter Sonata*"
      ],
      "time_hint": [0, 600],
...
> def search_beat(client, beat: dict, cast_filter: list[str] | None) - list[dict]:
    ...
    # Soft filter sequence: [cast+event] → [event] → [cast] → [none]
...
##### 4. expand — View the context before and after

- A 6-second hit isn’t a clip. We append the segment and dialogue from ±12 seconds (two segment slots) before and after.
- `select` provides the material for snapping the boundaries.
...
```python
Input:  beat="A Walk by the Lake and a Tender Glance at the Bus Stop" + candidates + context
...
Output:
{
  "usable": false,
  "score": 0.0,
  "reason": "The candidate's timeline contains only commentary and no actual scoring or batting footage",
  "retry_query": "KIA's Druw hit / KIA's Yukemi hit / ..."
}
```
...
- Only collect usable clips. Do not use an LLM.
...
# Trailers are listed in the order they were produced; the rest are listed in broadcast order and chronological order.
# The output's `v_id` plus `start_sec` divided by `end_sec` is passed as-is to the next step in FFmpeg
```
...
I’ve summarized the search flow and the process of structuring the `clips[]` array, which were the goals.
