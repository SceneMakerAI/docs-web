---
title: "07_Building a RAG Pipeline"
date: 2026-07-13T12:07:00
slug: 6
authors: [sbin]
description: "RAG—A Pipeline Builder That Folds Videos into Vectors (Indexing) and Retrieves Clips via Natural Language Queries (Search), Where Timecode Serves as the Axis of Symmetry"
tags: [rag, milvus]
last_update:
  date: 2026-07-23
---
...
: document structuring and parsing
2

; chunking into semantic units; and vectorization and embedding
3

. Fact-based Retrieval / Generating Answers:
...
- Fold segments into **semantic layers**. Index them as vectors.
  - 4-level division: Segment → Scene → Sub-scene → Overall Plot *(map-reduce)*
...
| --- | --- | --- | --- | --- |
| 0 | **loader** | Unpacking data | Merges dialogue (t_dialogue) and video segments (t_segment) from the video in chronological order | ❌ |
| 1 | **scene** | Split into scenes | Scene boundaries + summary + tags (highlights, emotions, ads) | ✅ Every 10-minute interval |
...
| 5 | **save** | Save notes | Record to DB | ❌ |
| 6 | **index** | Index for search | Embedding → 1 Milvus collection | ❌ |

In a nutshell, it’s all about **6-second atoms → folding into meaningful units → feeding into vectors**.
...
| --- | --- | --- | --- | --- |
| 1 | **scope** | Which video is it? | Selects only relevant episodes (40-vote chapter vector → confirmed by LLM, up to 5) | ✅ 0–1 times |
...
| 4 | **expand** | View the context | Attach a ±12-second context window to the top hits | ❌ |
| 5 | **select** | Is this really the right one? Where to cut? | Finalize clip segment [start–end] or discard (once per material) | ✅ 1–2 times per beat |
| 6 | **assemble** | Merge and adjust length | Merge overlaps → Trim to target length → Sort → Final list | ❌ |

The biggest difference from document-based RAG is **what happens after retrieval**.

Since a 6-second hit isn’t a clip, we append `expand` to the beginning and end, then use `select` to determine the section to cut.
...
*This post presents research results conducted with support from the Ministry of Science and ICT and the National IT Industry Promotion Agency (NIPA) under the “2026 Open-Source AI and Software Development and Utilization Support Project.”*
