...
In simple terms, the basic RAG process consists of the following three steps

<!--truncate-->

1. document structuring and parsing
2. chunking into semantic units; and vectorization (embedding)
3. Fact-based Retrieval / Generating Answers:

![image](/img/blog/07-rag-파이프라인-구축/img-00.png)

For document-based RAG, the process ends after quoting a paragraph in the third step.
...
- Fold segments into **semantic layers**. Index them as vectors.
  - 4-level partitioning: Segment → Scene → Sub-scene → Overall plot *(map-reduce)*
...
> 
>
> First, let’s just look at the flow for agent-scenario and agent-search; 
>
...
| --- | --- | --- | --- | --- |
| 0 | **loader** | Preprocessing | Merges dialogue (t_dialogue) and video segments (t_segment) from the video in chronological order | ❌ |
...
| 4 | **summary** | Write the Overall Plot | Combine acts into a single plot summary for the video (reduce) | ✅ Once |
| 5 | **save** | Save Notes | Record in DB | ❌ |
| 6 | **index** | Index for search | Embedding → 1 Milvus collection | ❌ |

In a nutshell, it’s all about **6-second atoms → folding into units of meaning → converting into vectors**.

The reason for folding a long video using Map-Reduce rather than summarizing it all at once—and the reason for logging timecodes at each step—is the same: the ultimate goal is **clip creation**.
...
Since the 6-second hit isn’t a clip, `expand` is appended to the beginning and end, and `select` determines the section to be cut.

If it doesn’t match, it’s discarded rather than forced to fill the gap (the material is processed once per beat).
