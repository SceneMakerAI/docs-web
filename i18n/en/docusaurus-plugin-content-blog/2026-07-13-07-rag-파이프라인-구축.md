...
; semantic unit segmentation (chunking) and vectorization (embedding)
3

. Fact-based Retrieval / Answer Generation.
...
| --- | --- | --- | --- | --- |
| 0 | **loader** | Loading data | Merges dialogue (t_dialogue) and screen segments (t_segment) from the video in chronological order | ❌ |
...
The reason for folding a long video using MapReduce rather than summarizing it all at once—and the reason for recording timecodes at each step—is the same: the ultimate goal is **clip production**.
...
| 4 | **expand** | View the context | Attach a ±12-second context window before and after the top hits | ❌ |
